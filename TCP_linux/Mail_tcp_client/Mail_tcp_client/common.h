...
public:
	Message_processor() {}
	Message_processor(const Message_processor&) = delete;
	virtual ~Message_processor() {}

	virtual void process(std::string message, Address from) = 0;
private:
...
private:
	...
	void listen_thread(int32 offset)
	{
		...
		while (!terminated)
		{
			...
			Address address;
			{
				...
				address = shared.addresses[offset];
			}
			this->processor->process(message.message, address);
		}
	...
	Message_processor* processor{ nullptr };
